...
`site/benchmarks/competitive.md`'s Trie section).
...
32-thread machine at points during this run; see
`site/benchmarks/competitive.md`'s methodology table for this project's
environment-disclosure policy). Two independent full runs of
...
| **Gap** | **~411× slower** at the short-word tier, narrowing (but still severe) as input grows: **~127×** at short phrase, **~42×** at sentence, **~16×** at paragraph — see "Likely reason" for why shrinking-with-length is exactly what the explanation below predicts, and see `site/benchmarks/competitive.md`'s Language detection Accuracy section for why this is *not* simply "whichlang is the better detector": lingua and Verbora both meaningfully out-accuracy whichlang at the short-word tier on this project's own 13-language test set (92.3%/76.9% vs. 69.2%). |
...
| **Gap** | Verbora **wins** `de` (1.0×–1.7× across sizes) and `nl` (1.7×–2.8×) but **loses** the other seven, consistently across every batch size (4 to 1024): `es` 0.16×–0.32×, `fr` 0.22×–0.66×, `it` 0.34×–0.38×, `no` 0.15×–0.22×, `pt` 0.22×–0.27×, `ru` **0.12×–0.14×** (worst), `sv` 0.15×–0.17×. |
...
**Update, later pass — implemented, not just recommended.** `verbora_spellcheck::DeletionIndex`/`DeletionIndexBuilder` now exist: a SymSpell-style deletion index built in-house with `verbora_distance` primitives, exactly as recommended (not by wrapping `fast_symspell` itself). Deletion generation operated, as first written, on **UTF-16 code units** rather than `char`s — matched to the granularity `verbora_distance::levenshtein` then verified distance at, since generating candidates at a coarser granularity than the verifier's would silently under-generate for astral (non-BMP) input; the same class of bug this crate's own edit generator already documented for `Spellcheck`. **That premise inverted, and the inconsistency it opened is now closed.** `verbora_distance::levenshtein` counts Unicode scalar values, not UTF-16 code units (`docs/design/distance-contract.md` §2), which made the verifier the *coarser* of the two for as long as generation stayed on code units: a deletion index keyed on code-unit deletions generates candidates that split an astral character in half — sequences the scalar-granularity verifier will never score at distance 1. That was recorded here as a real, open cross-crate inconsistency. It has since been resolved in the direction the contract requires: generation moved to scalars, `crates/verbora-spellcheck/src/units.rs` is gone, and the crate's `src/` now contains no `u16` at all — one metric, one unit, on both sides of the generate-then-verify step. The ASCII and BMP domains, which is what the numbers below measure, were unaffected either way. Correctness verified before any timing below was trusted: a 3,000-word ASCII sample vs. brute force at distance 0–3 (`tests/deletion_index.rs`), a dedicated astral-character-heavy dictionary vs. the same baseline (exercising the code-unit fix directly), and agreement with `FuzzyIndex` itself on a 1,000-word sample at every distance within `DeletionIndex`'s own build-time cap. Independently re-verified by a second, adversarial audit agent with no visibility into the implementation's own design reasoning.

**Real result: another genuine, honest trade-off, not a clean win.** ⚠ **Every figure in this paragraph is retired, pending re-measurement; do not cite any of them.** The index's map has since been re-keyed from the full deletion sequence onto a 64-bit hash of it, taking the cost of indexing one `n`-scalar word from cubic in `n` to quadratic and changing both timed paths, so these numbers describe a structure that no longer exists; the direction of the change must not be inferred in speed either. What survives is the shape alone, which follows from the two designs rather than from any measurement: dearer construction, bought against a query cost that grows far more slowly with corpus size, with a crossover somewhere small where the BK-tree still wins. Retained for provenance, and so the re-run has something to diff against. As recorded (`cargo bench -p verbora-spellcheck --bench deletion_index`, `max_distance=2`, same machine): construction — `DeletionIndex` is **13×–25× slower to build than `FuzzyIndex`** at every size (977.6 µs → 407.0 ms vs. 38.7 µs → 26.97 ms, n=100→20,000) — the same shape of cost `fast_symspell` itself pays against Verbora's plain `Spellcheck` above. Query — a genuine **crossover**: `FuzzyIndex` is actually faster at the smallest size (100 words, 1.73×, where a shallow BK-tree beats a deletion index's fixed per-query overhead), but `DeletionIndex` wins from 1,000 words up, the margin **widening rapidly** — 4.9× → 35.3× → 54.3× at 20,000 — near-flat growth with corpus size (3.3× over a 200× larger corpus) against `FuzzyIndex`'s roughly 300× growth over the same range, the same widening shape `fast_symspell` showed against `FuzzyIndex` in the comparison above. `DeletionIndex` also beats brute force by a wide, widening margin throughout (1.6× → 190×). Neither structure replaces the other: `FuzzyIndex` stays the default (cheaper, more predictable, no build-time distance cap); `DeletionIndex` earns its place for large (≥1,000-word), high-query-volume, fixed-`max_distance` workloads specifically — see `docs/PERFORMANCE_MATRIX.md`'s own `DeletionIndex` entry and `docs/COMPETITIVE_BENCHMARKS.md` §1.17's updated Architectural decision note for the full numbers and reasoning.

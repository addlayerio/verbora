...
| `verbora-tagger` | ✅ `tag_stream` | ✅ `TaggedToken` borrows the caller's tokens; nothing is copied to tag one | ✅ `tag_into` / `annotate_into` | ✅ | ✅ `par_tag_batch` | ⚠️ one `Tag` clone per rule application deferred (`transform`'s `words[i].tag = rule.to.clone()`; cheap in absolute terms) | ✅ | ➖ lexicon is caller-supplied at runtime, no bundled dataset | ⚠️ suite exists (`benches/tagger.rs`, rebuilt on generated inputs after the 0.3.0 data removal) but has not been run against it | ✅ |
